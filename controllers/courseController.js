...
exports.insertCourse = async (req, res) => {
  const { course_name, course_id, department } = req.body;
  // console.log("details " + course_name, course_id, department)
  if (!course_name || !course_id || !department) {
    console.log(course_name, course_id, department)
    return res.status(400).json({ message: 'All fields are required' });
  }
  const courseQuery = 'INSERT INTO courses (course_name, course_id, department) VALUES (?, ?, ?)';

  db.query(courseQuery, [course_name, course_id, department], (err, results) => {
    if (err) {
