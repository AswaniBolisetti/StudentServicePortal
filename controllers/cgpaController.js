...
const upload = multer({
    storage: multer.diskStorage({
        destination: (req, file, cb) => {
            const uploadPath = path.join(__dirname, 'studentsData');
            cb(null, uploadPath);
        },
...
exports.uploadCGPAFile = (req, res) => {
    const uploadSingle = upload.single('CGPAFile');
    ...
    uploadSingle(req, res, (err) => {
        ...
        const filePath = path.join(__dirname, 'studentsData',  req.file.filename);
...
exports.insertUsersFromExcel = async (req, res) => {
  try {
    ...
    data = data.filter((row) => row['Roll No'] && row['Name'] && row['CGPA']);
    ...
    const insertPromises = users.map((user) => {
      return new Promise((resolve, reject) => {
        const query = 'INSERT INTO cgpa (username, Name, CGPA) VALUES (?, ?, ?)';
        db.query(query, [user.roll_no, user.name, user.cgpa], (err, results) => {
          if (err) {
